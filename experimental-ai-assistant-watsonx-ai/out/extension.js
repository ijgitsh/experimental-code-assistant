...
function activate(context) {
    ...
    const processData2 = async (sdata, webview) => {
        ...
        try {
            const response = await axios_1.default.post(myUrl, postData, {
                headers: {
    ...
    const processData = async (mdata, webview) => {
        try {
            bearer_token = await authenticate();
        }
        catch (error) {
        ...
        if (editor) {
            ...
            try {
                ...
                if (workspaceFolders) {
                    ...
                    const newFilePath = path.join(workspacePath, userInput + "_converted");
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(MyCustomViewProvider.viewType, viewProvider));
